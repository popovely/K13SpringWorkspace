...
/*
	...
	컨트롤러 클래스 역할을 부여하기 위한 어노테이션으로,
	스프링 컨테이너가 시작될때 자동으로 빈이 생성된다.
 */
@Controller
public class ServiceController {
	/*
		서비스 객체를 주입받는다.
		@Autowired는 멤버변수, 생성자, setter에서 사용할 수 있다.
		서비스 객체가 미리 생성되어 있어야하고,
		그렇지 않으면 서버가 정상적으로 실행되지 않는다.
	 */
	MyService myService;
	@Autowired
	...
	public String myLotto(LottoVO lottoVO, Model model) {
		/*
			파라미터로 전달되는 값은 커맨드객체(VO객체)를 통해 한꺼번에 받을수 있으므로
			getParameter()가 필요없다.
		 */

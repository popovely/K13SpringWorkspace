...
public class FileuploadController {
	...
	/*
	파일업로드 처리
		: 파일업로드는 Post방식으로 전송해야 하므로
		  매핑시 method, value 두가지 속성을 모두 기술해야 한다.
	 */
	...
	public String uploadAction(Model model, MultipartHttpServletRequest req) {
		...
		// 파일정보를 저장한 Map컬렉션을 2개이상 저장하기 위한 List 컬렉션
...
// DB에 저장해야하지만 지금은 DB처리를 하지 않았으므로 Map에 저장한다.
				...
				// 하나의 파일정보를 저장한 Map을 List에 저장한다. (현재 파일이 2개이기 때문)

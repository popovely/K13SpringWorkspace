...
/*
	사용자의 요청을 제일 먼저 받는 DispatcherServlet은
	기본패키지로 지정된 com.kosmo.springlegacy를 스캔하여 컨트롤러 클래스를 찾아낸다.
	그리고 해당 요청명에 매핑된 메소드를 찾아서 요청을 전달하고 로직을 수행한다.
...
@RequestMapping
	: 요청명을 매핑한다.
	  ...
	  요청명의 매핑정보로 메소드를 호출하므로 메소드명은 큰 의미가 없다.
...
public class FormController {
	...
	/*
		[폼값받기2]
			...
			이와같이 사용하면 해당 메소드 내에서 변수명을 그대로 사용할 수 있다.
	 */
	...
	/*
		...
		조건1]
			쿼리스트링으로 전달되는 파라미터의 개수와 폼값을 저장할 객체 (DTO 혹은 VO)의 멤버변수의 개수가 동일한 것을 권장한다.
		조건2]
			커맨트객체를 사용할 때는 클래스명 앞글자를 소문자로 바꾼 형태의 매개변수를 사용해야 한다.
			Ex) MemberDTO	->	memberDTO
		조건3]
			커맨드객체를 만들때 파라미터의 개수는 달라도 상관없으나 파라미터명과 멤버변수명은 동일해야 하고
	...
	/*
		[폼값받기4]
			...
			파라미터를 마치 경로명처럼 사용하게되고,
			이 때 주의할 점은 웹브라우저는 요청명을 경로로 인식하므로 리소스를 사용할 때
			경로를 주의해야한다. 또한 파라미터의 개수가 다르면 404에러가 발생한다.
	 */
